...
__global__ 
void calculateHash(SHA1Context **sha,char *data,int len) 
{
    
    int i;

    SHA1Reset(sha[]);
    SHA1Input(sha, (const unsigned char *)data, len);
...
int main()
{
	...
	SHA1Context *sha_cpu;
	int i=0;
	const int csize = N*sizeof(SHA1Context);
	for(i=0;i<10;i++)
	cudaMalloc( (void**)&sha, csize ); 
	dim3 dimBlock( blocksize, 1 );
	dim3 dimGrid( 1, 1 );
	calculateHash<<<dimGrid, dimBlock>>>(sha);
	cudaMemcpy( sha_cpu, sha, csize, cudaMemcpyDeviceToHost ); 
	cudaFree(sha );
	//cudaFree( bd );
	
	for(i = 0; i < 5 ; i++)
        {
            printf("%X ", sha_cpu->Message_Digest[i]);
        }
	return 1;
}
